//
//  ItemInfoViewController+Extension.swift
//  CashBackApp
//
...
extension ItemInfoViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, UITableViewDelegate, UITableViewDataSource {
    ...
    //  Создаем collectionView для clickedItem.imageUrls
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: ItemPhotoCollectionViewCell.key, for: indexPath) as? ItemPhotoCollectionViewCell else { return
            UICollectionViewCell() } // присваиваем фото ячейкам коллекции
    ...
    func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) { //настроили пейдж контроллер
    ...
    //MARK: - Создаем tableView с инфо о товаре и предложениях
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        //MARK: - создаем кнопку для развертывания\скрывания предложений
        
        guard let actionsQty = clickedElement?.actions?.count else { return UIView() }
        if actionsQty > 1 { // проверили, чтобы было что разворачивать
    ...
    @objc
    private func hideRows(sender: UIButton) { //добавляем или удаляем элементы таблицы и рефрешим её
